...
async function createAndUploadFile(filePath, name) {
    ...
    switch(response.status){
        case 200:
            ...
            generatePublicUrl(response.data.id, name)
...
async function generatePublicUrl(fileId, name) {
    try {
      ...
      */
      ...
      fs.appendFileSync('links.txt', `${result.data.webContentLink} \n`);
      fs.appendFileSync('details.txt', `${name}: ${datetime}    =>    ${result.data.webContentLink} \n`);
